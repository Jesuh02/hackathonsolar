...
const capexSchema = z.object({
  minCop: z.number().nonnegative(),
  maxCop: z.number().nonnegative(),
  paybackMonths: z.number().nonnegative(),
  irr: z.number().optional(),   // TIR %/año
  npv: z.number().optional(),   // VAN COP (horizonte 20 años, descuento 12%)
  lcoe: z.number().optional(),  // COP/kWh nivelado
});

const scenariosSchema = z.object({
  conservador: z.string(),
  realista: z.string(),
  optimista: z.string(),
});
...
const recommendationItemSchema = z.object({
  ...
  impactType: z.enum(['economico', 'energetico', 'operativo']),
  ...
  savingsCopPerMonth: z.number().nonnegative(), // numeric savings for sum consistency
  savingsCalculationExplanation: z.string().optional(),
  capex: capexSchema.optional(),
  scenarios: scenariosSchema.optional(),
  benchmark: z.string().optional(),
  strategicOrder: z.number().int().min(1).optional(),
...
export class OpenRouterLlmAdapter implements LlmPort {
  ...
  constructor(apiKey: string, model: string, baseUrl: string) {
    this.httpClient = new HttpClient(baseUrl, 180000); // 180s timeout for LLM calls
...
- warnings top-level OBLIGATORIO: incluir el array estándar de 5 advertencias`;
    ...
    const irradianceSource = fin.todayIrradiance !== null
      ? `dato real del día ${fin.todayIrradiance.toFixed(2)} kWh/m²/día (solo contextual, NO usar en financieros)`
      : `dato de hoy no disponible aún en NASA POWER`;
...
${isPeak
  ? '⚡ HORA PICO SOLAR: máxima generación fotovoltaica.'
  : isDay
    ? '☀️ Generación solar activa (pico máximo: 10h–15h).'
    : '🌙 Noche: sin generación solar. Foco en baterías y programación de cargas.'}
...
11. En recomendación de demanda: añadir "Requiere validación con medición de curva de carga (15 min, 2 semanas)".`;;
  ...
  private buildFallbackRecommendation(request: LlmRecommendationRequest): Recommendation {
    ...
    const solarCapKw = profile.hasSolarPanels
      ? (profile.solarCapacityKw ?? Math.ceil(profile.peakDemandKw * 0.4))
      : Math.ceil(profile.peakDemandKw * 0.4);
    ...
    const recommendations: RecommendationItem[] = [
      ...
      },
      {
        ...
        description: `Irradiancia promedio histórica NASA POWER (P50): ${irr} kWh/m²/día. ${profile.hasSolarPanels
          ...
          : `Sistema propuesto: ${solarCapKw} kW. E = ${solarCapKw} kW × ${irr} kWh/m²/día × ${pr} PR × ${fin.lossesFactorCaribe} factor_Caribe = ${fin.potentialSolarGenKwhPerDay} kWh/día. Autoconsumo diurno (${profile.businessType}): ${autoconsumo}.`} El ahorro aplica exclusivamente a horas de producción solar (on-grid, sin generación nocturna).`,
        assumptions: [
          `Tarifa: ${profile.electricityRateCopPerKwh} COP/kWh`,
        ...
        action: profile.hasSolarPanels
          ? `Se recomienda operar cargas intensivas (aires, bombas, refrigeración) exclusivamente entre 10:00 y 15:00, maximizando autoconsumo diurno y reduciendo exportación a red. El sistema on-grid no produce energía de noche.`
          : `Se recomienda implementar un sistema solar fotovoltaico On-Grid de ${solarCapKw} kWp como medida estructural de reducción de costos energéticos. Inversión estimada: ${(solarCapKw * 3_500_000).toLocaleString('es-CO')}–${(solarCapKw * 5_500_000).toLocaleString('es-CO')} COP.`,
        ...
        capex: profile.hasSolarPanels ? undefined : {
          minCop: solarCapKw * 3_500_000,
          maxCop: solarCapKw * 5_500_000,
          // payback with degradation: CAPEX / (ahorro_año1 × 0.988) × 12
          paybackMonths: Math.round((solarCapKw * 4_500_000) / (Math.max(solarSavingsCop, 1) * 12 * 0.988)),
        },
        scenarios: {
      ...
      },
      {
        ...
        action: profile.hasBatteryStorage
          ? `Se recomienda cargar baterías entre 10h–15h con excedente solar (autoconsumo diurno). Descargar ante cortes o picos de demanda > ${Math.round(profile.peakDemandKw * 0.85)} kW. Evaluar si inyección a red (CREG 030) es más rentable que almacenar.`
          : `Si los apagones son frecuentes: se recomienda analizar banco de ${Math.max(Math.round(profile.operatingHoursPerDay * profile.peakDemandKw * 0.15), 4)} kWh (LiFePO4) para cargas críticas. Evaluar primero net metering antes de invertir en almacenamiento.`,
        ...
        capex: profile.hasBatteryStorage ? undefined : (() => {
          ...
          const capexMin = battKwh * 2_000_000; // 2 M COP/kWh
          const capexMax = battKwh * 3_000_000; // 3 M COP/kWh
    ...
    const estimatedSavings = Math.min(
      Math.round(totalSavingsCop / tariff),
      fin.maxSavingsCapKwh
    );
