...
export class ExcelReportAdapter implements ReportGeneratorPort {
  ...
  async generate(content: ReportContent): Promise<Buffer> {
    ...
    const [chartBuf, mapBuf] = await Promise.all([
      content.chartData
        ? this.chartService.getBarChartPng(
            ...
            content.chartData.title ?? 'Radiación Solar Promedio Mensual (kWh/m²/día)',
          )
    ...
    cover.getCell('A2').style = { fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: `FF${ACCENT_COLOR}` } } };
    ...
    cover.getCell('A3').style = {
      font: { bold: true, size: 16, color: { argb: `FF${HEADER_COLOR}` } },
      alignment: { horizontal: 'center', vertical: 'middle' },
    };
    ...
    cover.getCell('A4').style = {
      font: { italic: true, size: 12, color: { argb: 'FF555555' } },
    ...
    // Metadata
    const metaRows = [
      ['📍 Ubicación', content.location],
      ['🏢 Empresa', content.company ?? '—'],
      ['📅 Período', `${content.period.from}  →  ${content.period.to}`],
      ['🕐 Generado', content.generatedAt],
      ['🌍 Coordenadas', '11.5444°N, 72.9072°O'],
    ];
    metaRows.forEach(([label, value], i) => {
      ...
      row.getCell(1).style = {
        font: { bold: true, color: { argb: `FF${HEADER_COLOR}` } },
    ...
    if (chartBuf) {
      const chartId = workbook.addImage({ buffer: chartBuf, extension: 'png' });
      ...
      cover.getCell(`A${coverNextRow}`).style = {
        font: { bold: true, size: 13, color: { argb: `FF${HEADER_COLOR}` } },
        fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: `FF${HEADER_COLOR}` } },
        alignment: { horizontal: 'left', indent: 1, vertical: 'middle' },
      };
      cover.getCell(`A${coverNextRow}`).font = { bold: true, size: 13, color: { argb: 'FFFFFFFF' } };
      cover.getRow(coverNextRow).height = 32;
      coverNextRow++;

      cover.addImage(chartId, {
        tl: { col: 0, row: coverNextRow - 1 },
      ...
      // Source note
      cover.mergeCells(`A${coverNextRow}:H${coverNextRow}`);
      cover.getCell(`A${coverNextRow}`).value =
        'Fuente: NASA POWER API · Parámetro: ALLSKY_SFC_SW_DWN · Promedio mensual';
      cover.getCell(`A${coverNextRow}`).style = {
    ...
    // ── Map image in cover ───────────────────────────────────────────────────
    if (mapBuf) {
      const mapId = workbook.addImage({ buffer: mapBuf, extension: 'png' });
      cover.mergeCells(`A${coverNextRow}:H${coverNextRow}`);
      ...
      cover.mergeCells(`A${coverNextRow}:H${coverNextRow}`);
      cover.getCell(`A${coverNextRow}`).value =
        'Riohacha, capital de La Guajira — 11.5444°N, 72.9072°O. Fuente: © OpenStreetMap contributors.';
      cover.getCell(`A${coverNextRow}`).style = {
        font: { italic: true, size: 9, color: { argb: 'FF888888' } },
        alignment: { horizontal: 'center' },
      };
    ...
    if (content.chartData?.labels.length) {
      ...
      ['A2', 'B2'].forEach((addr) => {
        chartWs.getCell(addr).style = {
          ...
          fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: `FF${ACCENT_COLOR}` } },
          alignment: { horizontal: 'center' },
        };
      });
        ...
        if (i % 2 === 1) {
          ['A', 'B'].forEach((col) => {
            chartWs.getCell(`${col}${3 + i}`).fill = {
              type: 'pattern', pattern: 'solid', fgColor: { argb: `FF${ALT_ROW_COLOR}` },
            };
          });
        }
        r.height = 20;
    ...
    content.sections.forEach((section: ReportSection, si: number) => {
      ...
      if (section.keyMetrics?.length) {
        section.keyMetrics.forEach(({ label, value, unit }) => {
          ...
          ws.getCell(`A${rowIdx}`).style = { font: { bold: true, color: { argb: `FF${HEADER_COLOR}` } }, fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFEFF6FF' } } };
          ws.getCell(`B${rowIdx}`).style = { font: { bold: true, size: 13, color: { argb: `FF${ACCENT_COLOR}` } } };
      ...
      if (section.paragraphs?.length) {
        section.paragraphs.forEach((p) => {
          ...
          ws.getCell(`A${rowIdx}`).style = { alignment: { wrapText: true } };
    ...
    if (content.recommendations.length) {
      ...
      content.recommendations.forEach((rec, i) => {
        ...
        recWs.getCell(`A${i + 2}`).style = {
          ...
          fill: i % 2 === 1 ? { type: 'pattern', pattern: 'solid', fgColor: { argb: `FF${ALT_ROW_COLOR}` } } : undefined,
        };
