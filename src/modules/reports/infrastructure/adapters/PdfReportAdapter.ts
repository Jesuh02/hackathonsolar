...
export class PdfReportAdapter implements ReportGeneratorPort {
  ...
  async generate(content: ReportContent): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      const doc = new PDFDocument({
        size: 'A4',
        margins: { top: 60, bottom: 60, left: ML, right: 72 },
        autoFirstPage: true,
        // NO pageAdded listener anywhere — zero recursion risk
      });
      ...
      // New page + footer
      const newPage = () => { doc.addPage(); footer(); };

      // Ensure h pixels free; page-break if needed
      const need = (h: number) => { if (doc.y + h > Y_MAX) newPage(); };
      ...
      const sectionBar = (title: string) => {
        ...
        doc.fillColor('white').fontSize(13).font('Helvetica-Bold')
          .text(title, ML + 8, y + 12, { width: W - 16, lineBreak: false });
        doc.y = y + 52;
      };
      ...
      for (let r = 0; r < 8; r++) {
        const a = (r * Math.PI) / 4;
        doc.moveTo(PW / 2 + Math.cos(a) * 44, 76 + Math.sin(a) * 44)
          .lineTo(PW / 2 + Math.cos(a) * 56, 76 + Math.sin(a) * 56)
          .lineWidth(2.5).strokeColor(A).stroke();
      }

      doc.fillColor('white').fontSize(22).font('Helvetica-Bold')
        .text('AGENTE SOLAR', 0, 128, { width: PW, align: 'center', lineBreak: false });
      doc.fillColor('#F0E68C').fontSize(11).font('Helvetica')
        .text('Riohacha · La Guajira · Colombia', 0, 155, { width: PW, align: 'center', lineBreak: false });
      ...
      doc.fillColor(P).fontSize(13).font('Helvetica-Bold')
        .text(content.title, ML + 2, tbY + 7, { width: W + 12, lineBreak: false });
      doc.fillColor('#555').fontSize(10).font('Helvetica-Oblique')
        .text(content.subtitle, ML + 2, tbY + 28, { width: W + 12, lineBreak: false });

      // Metadata table
      doc.y = tbY + 94;
      const meta: [string, string][] = [
        ['Ubicacion', content.location],
        ...(content.company ? [['Empresa', content.company] as [string, string]] : []),
        ['Periodo', `${content.period.from}  -  ${content.period.to}`],
        ['Generado', content.generatedAt],
      ];
      meta.forEach(([k, v]) => {
        const ry = doc.y;
        doc.rect(ML, ry, W, 22).fill('#F8FAFC');
        doc.rect(ML, ry, 3, 22).fill(P);
        doc.fillColor(P).fontSize(9).font('Helvetica-Bold')
          .text(k + ':', ML + 7, ry + 6, { width: W / 2 - 16, lineBreak: false });
        doc.fillColor('#333').fontSize(9).font('Helvetica')
          .text(v, ML + W / 2, ry + 6, { width: W / 2 - 8, lineBreak: false });
        doc.y = ry + 28;
      });
      ...
      if (content.chartData?.labels.length) {
        ...
        let maxV = vals[0]; let minV = vals[0]; let maxI = 0; let minI = 0;
        ...
        doc.fillColor(P).fontSize(10).font('Helvetica-Bold')
          .text(`Mayor: ${maxLbl} (${maxV} kWh/m2/dia)   |   Menor: ${minLbl} (${minV} kWh/m2/dia)`,
            ML + 8, ky + 8, { width: W - 16, lineBreak: false });
        doc.fillColor('#444').fontSize(9).font('Helvetica')
          .text('Fuente: NASA POWER API · ALLSKY_SFC_SW_DWN · Promedio mensual',
            ML + 8, ky + 26, { width: W - 16, lineBreak: false });
        doc.y = ky + 54;
      }
        ...
        if (section.keyMetrics?.length) {
          ...
          section.keyMetrics.forEach(({ label, value, unit }) => {
            ...
            doc.fillColor('#555').fontSize(9).font('Helvetica')
              .text(label, ML + 8, ry + 4, { width: W / 2, lineBreak: false });
            doc.fillColor(P).fontSize(13).font('Helvetica-Bold')
              .text(`${value}${unit ? '  ' + unit : ''}`, ML, ry + 8, { width: W - 8, align: 'right', lineBreak: false });
            doc.y = ry + 36;
          });
          doc.moveDown(0.3);
        }
        ...
        if (section.paragraphs?.length) {
          section.paragraphs.forEach((p) => {
            ...
            doc.fillColor('#222').fontSize(11).font('Helvetica')
              .text(p, ML, doc.y, { width: W, align: 'justify' });
            doc.moveDown(0.4);
          });
        }
      ...
      if (content.recommendations.length) {
        ...
        content.recommendations.forEach((rec, i) => {
          ...
          doc.fillColor('white').fontSize(11).font('Helvetica-Bold')
            .text(`${i + 1}`, ML, bY + 5, { width: 24, align: 'center', lineBreak: false });
          doc.fillColor('#222').fontSize(10.5).font('Helvetica')
            .text(rec, ML + 30, bY + 5, { width: W - 30, lineBreak: false });
      ...
      if (content.conclusion) {
        ...
        doc.fillColor('#222').fontSize(11).font('Helvetica')
          .text(content.conclusion, ML + 10, cY, { width: W - 10, align: 'justify' });
      }
  ...
  private drawBarChart(doc: PDFKit.PDFDocument, labels: string[], values: number[], W: number, ML: number): void {
    const CHART_H = 155;
    ...
    for (let i = 0; i <= 5; i++) {
      ...
      doc.fillColor('#666').fontSize(7).font('Helvetica')
        .text((maxV * yf).toFixed(1), ML, ly - 4, { width: YLW - 4, align: 'right', lineBreak: false });
    }
    ...
    values.forEach((v, i) => {
      ...
      doc.fillColor(P).fontSize(6).font('Helvetica-Bold')
        .text(v.toFixed(1), bx - 2, by - 9, { width: barW + 4, align: 'center', lineBreak: false });
      doc.fillColor('#444').fontSize(6).font('Helvetica')
        .text(labels[i], bx - 3, CY + CHART_H + 3, { width: barW + 6, align: 'center', lineBreak: false });
    });
  ...
  private drawTable(
    ...
    headers.forEach((h, i) => {
      doc.fillColor('white').fontSize(9).font('Helvetica-Bold')
        .text(h, ML + i * colW + 4, y + 7, { width: colW - 8, align: 'center', lineBreak: false });
    });
    y += 24;

    // Data rows
    rows.forEach((row, ri) => {
      if (y > yMax) { newPage(); y = 80; }
      ...
      row.forEach((cell, ci) => {
        doc.fillColor('#222').fontSize(9).font('Helvetica')
          .text(String(cell), ML + ci * colW + 4, y + 4, {
            width: colW - 8,
