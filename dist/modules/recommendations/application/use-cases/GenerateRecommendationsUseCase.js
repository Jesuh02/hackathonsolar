...
Object.defineProperty(exports, "GenerateRecommendationsUseCase", {
    enumerable: true,
    get: function() {
        return GenerateRecommendationsUseCase;
    }
});
const _EnergyProfile = require("../../domain/entities/EnergyProfile");
...
let GenerateRecommendationsUseCase = class GenerateRecommendationsUseCase {
    async execute(request) {
        ...
        if (request.analysisDatetime) {
            ...
            analysisDate = new Date(new Date().toLocaleString('en-US', {
        ...
        const energyProfile = new _EnergyProfile.EnergyProfile({
        ...
        if (solarResult.isFailure) {
            return _Result.Result.fail(solarResult.error);
        }
        ...
        const locationLabel = request.location ? request.location.address ? `${request.location.address} (${lat.toFixed(5)}, ${lon.toFixed(5)})` : `Riohacha, La Guajira, Colombia — coords (${lat.toFixed(5)}, ${lon.toFixed(5)})` : 'Riohacha, La Guajira, Colombia';
        ...
        const autoconsumoFactor = GenerateRecommendationsUseCase.autoconsumoByType[request.businessType] ?? 0.75;
        ...
        const potentialSolarSavingsCopPerMonth = Math.round(potentialSolarGenKwhPerDay * 30 * autoconsumoFactor * request.electricityRateCopPerKwh);
        ...
        const season = analysisMonth >= 11 || analysisMonth <= 4 ? 'temporada seca (irradiancia alta, nov–abr)' : 'temporada de lluvias (irradiancia moderada, may–oct)';
        // ── Invoke LLM ─────────────────────────────────────────────────────────
        ...
        if (llmResult.isFailure) {
            return _Result.Result.fail(llmResult.error);
        }
    ...
    async getSolarData(_startDate, endDate, lat, lon) {
        ...
        if (!bypassCache && cached && Date.now() - cached.fetchedAt < GenerateRecommendationsUseCase.CACHE_TTL_MS) {
            ...
            if (apiData.isFailure) return _Result.Result.fail(apiData.error);
            await this.solarRepository.save(apiData.value);
            allData = apiData.value.map((d)=>({
                    date: d.date,
                    irradiance: d.irradiance
                }));
            GenerateRecommendationsUseCase.historicalCache.set(cacheKey, {
                data: allData,
                fetchedAt: Date.now()
            });
        }
        ...
        const todayDateLabel = todayRecord ? `${todayRecord.date.slice(0, 4)}-${todayRecord.date.slice(4, 6)}-${todayRecord.date.slice(6, 8)}` : endDate;
        ...
        const yearlyAverageLines = Array.from(byYear.entries()).filter(([, vals])=>vals.length > 300).sort(([a], [b])=>a.localeCompare(b)).map(([yr, vals])=>{
        ...
        const summary = [
            ...
            ``,
            todayIrradiance !== null ? `Dato más reciente NASA POWER — ${todayDateLabel}: ${todayIrradiance.toFixed(2)} kWh/m²/día (referencia contextual, NO usar para financieros)` : `Dato del día actual aún no disponible en NASA POWER`,
            `Nivel de radiación: ${this.classifyIrradiance(p50Irradiance)}`,
            `Potencial solar: MUY ALTO (zona tropical, costa Caribe)`
        ].join('\n');
        return _Result.Result.ok({
            summary,
            avgIrradiance: parseFloat(avg30.toFixed(2)),
            p50Irradiance,
            historicalMin,
            historicalMax,
            rawDailyData,
    ...
    classifyIrradiance(avg) {
        if (avg >= 6) return 'excelente';
        if (avg >= 4.5) return 'alta';
        if (avg >= 3) return 'media';
        return 'baja';
    }
    /** Riohacha sunrise ~05:30, sunset ~18:30 (UTC-5, tropical) */ classifySolarConditionByHour(hour) {
    ...
    constructor(llmPort, solarRepository, nasaApi){
        _define_property(this, "llmPort", void 0);
        _define_property(this, "solarRepository", void 0);
...
_define_property(GenerateRecommendationsUseCase, "historicalCache", new Map());
_define_property(GenerateRecommendationsUseCase, "CACHE_TTL_MS", 6 * 60 * 60 * 1000);
/** Autoconsumo factor by business type (fraction of solar generation consumed on-site) */ _define_property(GenerateRecommendationsUseCase, "autoconsumoByType", {
    hotel: 0.85,
    hielera: 0.80,
    retail: 0.75,
    oficina: 0.70,
...
//# sourceMappingURL=GenerateRecommendationsUseCase.js.map