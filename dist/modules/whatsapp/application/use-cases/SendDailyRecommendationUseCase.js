...
Object.defineProperty(exports, "SendDailyRecommendationUseCase", {
    enumerable: true,
    get: function() {
        return SendDailyRecommendationUseCase;
    }
});
...
/** Static profile for Panadería Don Humberto - Riohacha */ const COMPANY_PROFILE = {
...
let SendDailyRecommendationUseCase = class SendDailyRecommendationUseCase {
    async execute() {
        if (!this.companyPhone) {
            return _Result.Result.fail('WHATSAPP_COMPANY_PHONE no está configurado');
        }
        ...
        if (recResult.isFailure) {
            return _Result.Result.fail(`No se pudieron generar recomendaciones: ${recResult.error}`);
        }
        // Fetch today's irradiance from NASA POWER API
        const { lat, lng } = COMPANY_PROFILE.location;
        let todayIrradiance = null;
        let avg30Irradiance = null;
        try {
            ...
            if (solarResult.isSuccess && solarResult.value.length > 0) {
                ...
                avg30Irradiance = parseFloat((data.reduce((s, d)=>s + d.irradiance, 0) / data.length).toFixed(2));
            }
        } catch  {
        ...
        if (todayIrradiance !== null) {
            const trend = avg30Irradiance !== null ? todayIrradiance >= avg30Irradiance ? `▲ sobre el promedio (${avg30Irradiance} kWh/m²/día)` : `▼ bajo el promedio (${avg30Irradiance} kWh/m²/día)` : '';
    ...
    constructor(whatsApp, generateRecommendations, companyPhone, nasaApi){
        _define_property(this, "whatsApp", void 0);
        _define_property(this, "generateRecommendations", void 0);
        _define_property(this, "companyPhone", void 0);
...
//# sourceMappingURL=SendDailyRecommendationUseCase.js.map